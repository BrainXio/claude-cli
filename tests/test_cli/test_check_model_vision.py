...
def test_get_active_model_with_ollama_cloud():
    ...
    from claude_cli.check_model_vision import get_active_model, STATE_FILE

    state = {
        "mode": {
            "string": "Ollama-Cloud:deepseek-v4-pro:cloud"
        }
    }
...
def test_main_empty_env_vars():
    ...
    from claude_cli.check_model_vision import main, CAP_FILE
    import io
...
def test_main_active_model():
    ...
    from claude_cli.check_model_vision import main, STATE_FILE
    import io
...
def test_check_model_vision_with_ollama_show():
    ...
    from claude_cli.check_model_vision import check_model_vision, has_vision_capability

    with patch("claude_cli.check_model_vision.ollama_show") as mock_show:
        mock_show.return_value = ("Capabilities\n  vision", True)
        with patch("claude_cli.check_model_vision.has_vision_capability") as mock_vision:
    ...
    with patch("claude_cli.check_model_vision.ollama_show") as mock_show:
        ...
        with patch("claude_cli.check_model_vision.ollama_tags") as mock_tags:
            mock_tags.return_value = [
                {"name": "llama3", "remote_model": "llama3"}
            ]
            with patch("claude_cli.check_model_vision.has_vision_capability") as mock_vision:
                mock_vision.return_value = True
                vision, matched = check_model_vision("llama3")
...
def test_check_model_vision_known_remote():
    ...
    with patch("claude_cli.check_model_vision.ollama_show") as mock_show:
        ...
        with patch("claude_cli.check_model_vision.ollama_tags") as mock_tags:
            mock_tags.return_value = [
                {"name": "qwen-vl", "remote_model": "qwen2.5-vl"}
            ]
...
def test_main_with_env_var():
    ...
    with patch.dict(os.environ, {"ANTHROPIC_DEFAULT_SONNET_MODEL": "claude-3-5-sonnet"}):
        with patch("os.path.exists", return_value=False):
            with patch("os.makedirs"):
                with patch("claude_cli.check_model_vision.check_model_vision") as mock_check:
                    mock_check.return_value = (None, None)
...
def test_main_active_model():
    ...
    with patch("os.path.exists", return_value=True):
        with patch("os.makedirs"):
            with patch("builtins.open") as mock_open:
                mock_file = MagicMock()
                mock_file.read.return_value = '{"mode": {"string": "Ollama-Cloud:granite-guardian:latest"}}'
                mock_open.return_value.__enter__.return_value = mock_file
                with patch("claude_cli.check_model_vision.check_model_vision") as mock_check:
                    mock_check.return_value = (None, None)
                    main()
