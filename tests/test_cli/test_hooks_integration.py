...
class TestBootstrapHook:
    ...
    def test_bootstrap_writes_state(
        ...
        monkeypatch.setattr(
            "claude_cli.bootstrap.STATE_FILE", tmp_path / "data" / "state.json"
        )
        monkeypatch.setattr(
            "claude_cli.bootstrap.REPORTS_LOGS", tmp_path / "reports" / "logs"
        )
        monkeypatch.setattr(
            "claude_cli.bootstrap.REPORTS_STATE", tmp_path / "reports" / "state"
        )
        monkeypatch.setattr(
            "claude_cli.bootstrap.REPORTS_TMP", tmp_path / "reports" / "tmp"
        )
        ...
        with patch(
            "claude_cli._bootstrap_identity.register_and_resume",
            return_value=(mock_record, [], 0),
        ):
    ...
    def test_bootstrap_creates_directories(
        ...
        # Also patch workspace-local directories
        monkeypatch.setattr(
            "claude_cli.bootstrap.DAILY_DIR", tmp_path / "data" / "daily"
        )
        monkeypatch.setattr(
            "claude_cli.bootstrap.KNOWLEDGE_DIR", tmp_path / "data" / "kb"
        )
        monkeypatch.setattr("claude_cli.bootstrap.WORKSPACE_ROOT", tmp_path)
...
class TestSessionEndHook:
    ...
    def test_session_end_empty_transcript(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        spawned_cmds: list[list[str]] = []
        monkeypatch.setattr(
            "claude_cli.session_end.spawn_detached",
            lambda cmd, cwd=None: spawned_cmds.append(cmd),
        )
...
class TestStandardsGuardHook:
    ...
    def test_guard_blocks_philosophy_sludge(
    ...
    def test_guard_ignores_non_guarded_files(
        self, capsys: pytest.CaptureFixture
    ) -> None:
...
class TestCheckModelVisionHook:
    ...
    def test_vision_detected_via_ollama_show(
    ...
    def test_vision_false_when_not_in_caps(
        self, capsys: pytest.CaptureFixture, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_fallback_to_known_vision_remote(
        self, capsys: pytest.CaptureFixture, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_empty_env_outputs_nothing(
        self, capsys: pytest.CaptureFixture, monkeypatch: pytest.MonkeyPatch
    ) -> None:
